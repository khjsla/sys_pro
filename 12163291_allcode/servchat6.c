...
void handle_state_4(int x, fd_set *pset, char *buf, int state[]){
	int y;
	y=find_socket(buf);
	printf("state 4 for %s. partner name:%s\n", cli[x].name, buf); 
	printf("found partner:%d\n", y);
	cli[x].partner=y;
	write(x, "start chatting", 14);
	state[x]=5;
}
void handle_state_5(int x, fd_set *pset, char *buf, int state[]){
	int partner=cli[x].partner;
	char msg[128];
	sprintf(msg,"%s to %s:%s\n", cli[x].name,cli[partner].name,buf); 
	write(partner, msg, strlen(msg));
}
